...
server.get(`${ROOT}/user/scoreData`, async function (req, res) {
    res.status(200).send(require('./mocks/jsons/userScoreData.json'));
});
...
server.get(`${ROOT}/btcPrice/currentPrice`, async function (req, res) {
    // await delay(3000);
    test = Math.random();
    res.status(200).send(test.toString());
    //res.status(200).send(require('./mocks/jsons/price.json'));
});
